...
import ServerBlast from '@server/rooms/schema/Blast';
import ServerBlock from '@server/rooms/schema/Block';
import { Bomb as ServerBomb } from '@server/rooms/schema/Bomb';
import GameResult from '@server/rooms/schema/GameResult';
import GameRoomState from '@server/rooms/schema/GameRoomState';
import ServerItem from '@server/rooms/schema/Item';
import ServerPlayer from '@server/rooms/schema/Player';
import ServerTimer from '@server/rooms/schema/Timer';
...
export default class Network {
  ...
  constructor() {
    ...
    // Colyseus は Next.js のカスタムサーバーに同居しているため、同一オリジンに接続する。
    // 本番で別ホストを指定したい場合は NEXT_PUBLIC_SERVER_URL を設定する。
    const endpoint =
      Config.SERVER_URL && Config.SERVER_URL.length > 0
        ? Config.SERVER_URL
